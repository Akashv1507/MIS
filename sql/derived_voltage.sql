...
select trunc(vt.time_stamp) date_key,max(mt.ID)mapping-id ,vt.node_scada_name,min(mt.node_name)Node_name,min(vt.voltage_value) minimum, max(vt.voltage_value) maximum, avg(vt.voltage_value) average
